...
from ce.cte.utils import validate_string
...
class __TextToObject(CTEVisitor):
    ...
    def visitValueInt(self, ctx):
        ...
        if ctx.PINT_BIN():
            return int(ctx.PINT_BIN().getText().lstrip("0b"), 2)

        if ctx.NINT_BIN():
            return -int(ctx.NINT_BIN().getText().lstrip("-0b"), 2)

        if ctx.PINT_OCT():
            return int(ctx.PINT_OCT().getText().lstrip("0o"), 8)

        if ctx.NINT_OCT():
            return -int(ctx.NINT_OCT().getText().lstrip("-0o"), 8)

        if ctx.PINT_HEX():
            return int(ctx.PINT_HEX().getText().lstrip("0x"), 16)

        if ctx.NINT_HEX():
            return -int(ctx.NINT_HEX().getText().lstrip("-0x"), 16)
    ...
    def visitValueFloat(self, ctx):
        ...
        if ctx.FLOAT_DEC():
            ...
            text = ctx.FLOAT_DEC().getText().replace('_', '.').replace(",", ".")
            value = float(text)
        ...
        if ctx.FLOAT_HEX():
            ...
            text = ctx.FLOAT_HEX().getText().replace('_', '.').replace(",", ".")
