...
class ContainerMapTestCase(unittest.TestCase):
    ...
    def test_nested_map(self):
        self.assertEqual(
            cte.load("""c1 {"a" = {"b" = "c"} }"""), Map([("a", Map([("b", "c")]))])
        )
    ...
    def test_numeric_cte(self):
        ...
        filename = os.path.join(os.path.dirname(__file__), "examples/numeric_map.cte")
    ...
    def test_strings_only(self):
        ...
        filename = os.path.join(os.path.dirname(__file__), "examples/strings_only.cte")

        expected = Map(
            [
                ("string", "Strings support escape sequences: \n \t 🐕"),
                ("url", Rid('https://example.com/')),
                ("email", Rid('mailto:me@somewhere.com')),
            ]
        )
        with open(filename, "r") as f:
            test = f.read()
